...
import { Scheduler } from "@actalk/anywrite-core";
import { loadConfig, findProjectRoot, buildPipelineConfig, log, logError } from "../utils.js";
import { createWriteStream, type WriteStream } from "node:fs";
import { writeFile, readFile, unlink } from "node:fs/promises";
import { join } from "node:path";

const PID_FILE = "anywrite.pid";

export const upCommand = new Command("up")
  .description("Start the AnyWrite daemon (autonomous mode)")
  ...
  .action(async (opts) => {
    ...
    try {
      ...
      try {
        ...
        logError(`Daemon already running (PID: ${existingPid.trim()}). Run 'anywrite down' first.`);
        process.exit(1);
      } catch {
        // No PID file, good
      }

      log("Starting AnyWrite daemon...");
...
export const downCommand = new Command("down")
  .description("Stop the AnyWrite daemon")
  .action(async () => {
    const root = findProjectRoot();
