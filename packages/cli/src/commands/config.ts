...
import { listModelsForService } from "@actalk/anywrite-core";
...
configCommand
  .command("set-global")
  .description("Set global LLM config (~/.anywrite/.env), shared by all projects")
  ...
  .action(async (opts) => {
    try {
      ...
      const lines = [
        "# AnyWrite Global LLM Configuration",
        `ANYWRITE_LLM_PROVIDER=${opts.provider}`,
        `ANYWRITE_LLM_BASE_URL=${opts.baseUrl}`,
        `ANYWRITE_LLM_API_KEY=${opts.apiKey}`,
        `ANYWRITE_LLM_MODEL=${opts.model}`,
      ];
      if (opts.temperature) lines.push(`ANYWRITE_LLM_TEMPERATURE=${opts.temperature}`);
      if (opts.thinkingBudget) lines.push(`ANYWRITE_LLM_THINKING_BUDGET=${opts.thinkingBudget}`);
      if (opts.apiFormat) lines.push(`ANYWRITE_LLM_API_FORMAT=${opts.apiFormat}`);
      if (opts.lang) lines.push(`ANYWRITE_DEFAULT_LANGUAGE=${opts.lang}`);
...
configCommand
  .command("show-global")
  .description("Show global LLM config (~/.anywrite/.env)")
  .action(async () => {
    try {
      const content = await readFile(GLOBAL_ENV_PATH, "utf-8");
      const masked = content.replace(
        /(ANYWRITE_LLM_API_KEY=)(.{8})(.*)(.{4})/,
        "$1$2...$4",
      );
      log(masked);
    } catch {
      log("No global config found. Run 'anywrite config set-global' to create one.");
    }
  });
...
configCommand
  ...
  .option("--api-key <key>", "API Key (also reads from ANYWRITE_LLM_API_KEY env)")
  ...
  .action(async (service: string, opts: { apiKey?: string; baseUrl?: string; json?: boolean }) => {
    const apiKey = opts.apiKey ?? process.env.ANYWRITE_LLM_API_KEY;
