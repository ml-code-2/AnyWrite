...
export const doctorCommand = new Command("doctor")
  ...
  .action(async (opts: { repairNodeRuntime?: boolean }) => {
    ...
    // 2. Check anywrite.json exists
    try {
      await readFile(join(root, "anywrite.json"), "utf-8");
      checks.push({ name: "anywrite.json", ok: true, detail: "Found" });
    } catch {
      checks.push({ name: "anywrite.json", ok: false, detail: "Not found. Run 'anywrite init'" });
    }
    ...
    // 4. Check global config
    {
      ...
      try {
        ...
        hasGlobal = globalContent.includes("ANYWRITE_LLM_API_KEY=") && !globalContent.includes("your-api-key-here");
      } catch { /* no global config */ }
      checks.push({
        name: "Global Config",
        ok: hasGlobal,
        detail: hasGlobal ? `Found (${GLOBAL_ENV_PATH})` : "Not set. Run 'anywrite config set-global'",
      });
    }
    ...
    // 5. Check effective LLM config (Studio project base + env/CLI overlay, or legacy env)
    {
      ...
      const { isApiKeyOptionalForEndpoint } = await import("@actalk/anywrite-core");
    ...
    try {
      const { StateManager } = await import("@actalk/anywrite-core");
      ...
      if (hasStructuredState) {
        ...
        if (legacyCount > 0) {
          checks.push({
            ...
            ok: false,
            detail: `${legacyCount} book(s) using legacy format (pre-v0.6). Run 'anywrite write next' on each to auto-migrate, or re-init with 'anywrite init'.`,
          });
    ...
    try {
      const { createLLMClient, chatCompletion, LLMConfigSchema, isApiKeyOptionalForEndpoint, resolveServiceModelsBaseUrl } = await import("@actalk/anywrite-core");
      ...
      try {
        ...
        const apiKeyOptional = isApiKeyOptionalForEndpoint({
          provider: env.ANYWRITE_LLM_PROVIDER,
          baseUrl: env.ANYWRITE_LLM_BASE_URL,
        });
        if ((env.ANYWRITE_LLM_API_KEY || apiKeyOptional) && env.ANYWRITE_LLM_BASE_URL && env.ANYWRITE_LLM_MODEL) {
          llmConfig = LLMConfigSchema.parse({
            provider: env.ANYWRITE_LLM_PROVIDER ?? "custom",
            baseUrl: env.ANYWRITE_LLM_BASE_URL,
            apiKey: env.ANYWRITE_LLM_API_KEY ?? "",
            model: env.ANYWRITE_LLM_MODEL,
          });
        }
      }
      ...
      if (errMsg.includes("Connection error") || errMsg.includes("ECONNREFUSED") || errMsg.includes("fetch failed")) {
        hints.push("baseUrl 可能不正确，检查 ANYWRITE_LLM_BASE_URL 是否包含完整路径（如 /v1）");
      }
      if (errMsg.includes("400")) {
        hints.push("检查提供方文档，确认该接口要求 stream=true、stream=false，还是根本不支持 stream");
        hints.push("检查模型名称是否正确（ANYWRITE_LLM_MODEL）");
      }
      if (errMsg.includes("401")) {
        hints.push("API Key 无效，检查 ANYWRITE_LLM_API_KEY");
      }
