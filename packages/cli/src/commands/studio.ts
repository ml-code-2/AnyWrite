...
export async function resolveStudioLaunch(root: string): Promise<StudioLaunchSpec | null> {
  ...
  const builtEntry = await firstAccessiblePath([
    join(root, "node_modules", "@actalk", "anywrite-studio", "dist", "api", "index.js"),
    join(root, "node_modules", "@actalk", "anywrite-studio", "server.cjs"),
    join(cliPackageRoot, "node_modules", "@actalk", "anywrite-studio", "dist", "api", "index.js"),
    join(cliPackageRoot, "node_modules", "@actalk", "anywrite-studio", "server.cjs"),
    join(cliPackageRoot, "..", "anywrite-studio", "dist", "api", "index.js"),
    join(cliPackageRoot, "..", "anywrite-studio", "server.cjs"),
  ]);
  if (builtEntry) {
    return {
...
export async function launchStudioWorkbench(root: string, port: string): Promise<void> {
  ...
  if (!launch) {
    logError(
      "AnyWrite Studio not found. If you cloned the repo, run:\n" +
      "  cd packages/studio && pnpm install && pnpm build\n" +
      "Then run 'anywrite studio' from the project root.",
    );
    process.exit(1);
  }

  log(`Starting AnyWrite Studio on ${url}`);

  const child = spawn(launch.command, launch.args, {
    cwd: root,
    stdio: "inherit",
    env: { ...process.env, ANYWRITE_STUDIO_PORT: port },
  });
...
export async function launchStudioEntry(
  ...
  if (prepared.initialized) {
    log(`No anywrite.json found in ${root}. Initialized a minimal AnyWrite project for Studio.`);
  }
...
export function createStudioCommand(hooks: StudioCommandHooks = {}): Command {
  ...
  .description("Start AnyWrite Studio web workbench")
