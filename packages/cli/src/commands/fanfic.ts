...
import { PipelineRunner, type BookConfig, type FanficMode } from "@actalk/anywrite-core";
    ...
    try {
      ...
      if (opts.json) {
        log(JSON.stringify({
          ...
          nextStep: `anywrite write next ${bookId}`,
        }, null, 2));
      } else {
        ...
        log("");
        log(`Next: anywrite write next ${bookId}`);
      }
    } catch (e) {
      if (opts.json) {
  ...
  .action(async (bookIdArg: string | undefined, opts) => {
    try {
      ...
      try {
        ...
        throw new Error(`该书没有同人正典文件。用 anywrite fanfic init 创建同人书。`);
      }
