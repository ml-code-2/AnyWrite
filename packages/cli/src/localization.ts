import { formatLengthCount, resolveLengthCountingMode } from "@actalk/anywrite-core";
...
export function formatBookCreateNextStep(language: CliLanguage, bookId: string): string {
  return localize(language, {
    zh: `下一步：anywrite write next ${bookId}`,
    en: `Next: anywrite write next ${bookId}`,
  });
}
...
export function formatImportChaptersComplete(
  ...
  return [
    ...
    "",
    localize(language, {
      zh: `运行 "anywrite write next ${result.continueBookId}" 继续写作。`,
      en: `Run "anywrite write next ${result.continueBookId}" to continue writing.`,
    }),
  ];
}
