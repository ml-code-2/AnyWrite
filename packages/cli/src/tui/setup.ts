...
export function buildAutoInitMessages(projectName: string, locale: TuiLocale): {
  ...
  if (locale === "en") {
    return {
      ...
      envTemplateHeader: "# LLM Configuration — run anywrite tui to configure interactively",
    };
  }
  ...
  return {
    ...
    initialized: "项目已初始化",
    envTemplateHeader: "# LLM 配置 —— 运行 anywrite tui 进行交互式配置",
  };
}

export async function ensureProject(cwd: string): Promise<SetupResult> {
  const configPath = join(cwd, "anywrite.json");
...
export async function interactiveLlmSetup(
  ...
  try {
    ...
    const envContent = [
      `ANYWRITE_LLM_PROVIDER=${provider}`,
      `ANYWRITE_LLM_BASE_URL=${baseUrl.trim()}`,
      `ANYWRITE_LLM_API_KEY=${apiKey.trim()}`,
      `ANYWRITE_LLM_MODEL=${model.trim()}`,
    ].join("\n");
...
async function autoInit(cwd: string): Promise<void> {
  ...
  const config = {
    ...
    language: "zh",
    llm: {
      provider: process.env.ANYWRITE_LLM_PROVIDER ?? "openai",
      baseUrl: process.env.ANYWRITE_LLM_BASE_URL ?? "",
      model: process.env.ANYWRITE_LLM_MODEL ?? "",
    },
    notify: [],
    daemon: {
      schedule: {
...
async function checkEnvForKey(envPath: string): Promise<boolean> {
  try {
    ...
    const match = content.match(/ANYWRITE_LLM_API_KEY=(.+)/);
...
export async function detectProjectLanguage(projectRoot: string): Promise<string | undefined> {
  try {
    const raw = await readFile(join(projectRoot, "anywrite.json"), "utf-8");
...
async function parseEnvModel(envPath: string): Promise<ModelInfo | undefined> {
  try {
    ...
    const key = get("ANYWRITE_LLM_API_KEY");
    if (!key || key.includes("your-api-key")) return undefined;
    return {
      provider: get("ANYWRITE_LLM_PROVIDER") || "openai",
      model: get("ANYWRITE_LLM_MODEL") || "unknown",
      baseUrl: get("ANYWRITE_LLM_BASE_URL") || "",
    };
  } catch {
    return undefined;
  }
