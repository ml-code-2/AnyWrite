...
const ZH_CN: TuiCopy = {
  ...
  composer: {
    placeholder: "告诉 AnyWrite 要写什么、修改什么，或解释什么…",
    emptyConversation: "先告诉 AnyWrite 你要做什么。",
  ...
  notes: {
    ...
    config: "当前 Ink 仪表盘里还不支持交互式 /config。请使用 anywrite config set-global。",
    ...
    toolInitHint: "请检查 .env，或运行：anywrite config set-global",
  },
  roles: {
    user: "你",
    assistant: "AnyWrite",
...
const EN: TuiCopy = {
  ...
  composer: {
    placeholder: "Ask AnyWrite to write, revise, or explain…",
    emptyConversation: "Start by asking AnyWrite what to do.",
  ...
  notes: {
    ...
    config: "Interactive /config is not available inside the Ink dashboard yet. Use anywrite config set-global.",
    ...
    toolInitHint: "Check your .env or run: anywrite config set-global",
  },
  roles: {
    user: "You",
    assistant: "AnyWrite",
...
export function resolveTuiLocale(
  ...
  const requested = normalizeLocale(env.ANYWRITE_TUI_LOCALE ?? env.ANYWRITE_LOCALE);
  if (requested) {
    return requested;
  }
