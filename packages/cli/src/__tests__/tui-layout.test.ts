...
describe("tui layout", () => {
  it("renders a codex-like single-column workspace preview", () => {
    const frame = renderTuiFrame({
      locale: "zh-CN",
      projectName: "anywrite-demo",
      activeBookTitle: undefined,
      automationMode: "semi",
      status: "idle",
    });

    expect(frame).toContain("项目 anywrite-demo");
    ...
    expect(frame).toContain("告诉 AnyWrite");
  });
  ...
  it("keeps the two-line status strip above the composer preview", () => {
    ...
    expect(frame.indexOf("task.completed: Completed write_next for harbor.")).toBeLessThan(frame.indexOf("Ask AnyWrite"));
    expect(frame.indexOf("Mode auto")).toBeLessThan(frame.indexOf("Ask AnyWrite"));
  });
