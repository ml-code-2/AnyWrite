...
describe.sequential("publish packaging", () => {
  it("rewrites workspace package versions for canary publishing", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "anywrite-version-script-"));
    ...
    try {
      ...
      await writeFile(
        ...
        `${JSON.stringify({ name: "anywrite", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@actalk/anywrite-core", version: "0.4.6" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
          {
            name: "@actalk/anywrite",
            version: "0.4.6",
            dependencies: {
              "@actalk/anywrite-core": "workspace:*",
              commander: "^13.0.0",
            },
          },
          null,
      ...
      expect(cliPackageJson.dependencies["@actalk/anywrite-core"]).toBe("0.4.8-canary.7");
    } finally {
  ...
  it("keeps source CLI dependencies linked through the workspace protocol", async () => {
    ...
    expect(cliPackageJson.dependencies["@actalk/anywrite-core"]).toBe("workspace:*");
    expect(cliPackageJson.dependencies["@actalk/anywrite-studio"]).toBe("workspace:*");
  });
  ...
  it("allows source workspace protocol manifests when they normalize cleanly for publish", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "anywrite-publish-verify-pass-"));
    const tempPackagesDir = join(tempRoot, "packages");
    const tempCoreDir = join(tempPackagesDir, "core");
    const tempCliDir = join(tempPackagesDir, "cli");
    ...
    try {
      ...
      await writeFile(
        join(tempRoot, "package.json"),
        `${JSON.stringify({ name: "anywrite", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCoreDir, "package.json"),
        `${JSON.stringify({ name: "@actalk/anywrite-core", version: "0.5.1" }, null, 2)}\n`,
      );
      await writeFile(
        join(tempCliDir, "package.json"),
        `${JSON.stringify(
  ...
  it("rejects workspace protocol manifests that normalize to the wrong internal version", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "anywrite-publish-verify-fail-"));
    const tempPackagesDir = join(tempRoot, "packages");
    const tempCoreDir = join(tempPackagesDir, "core");
    const tempCliDir = join(tempPackagesDir, "cli");
        ...
        `${JSON.stringify(
          {
            ...
            version: "0.5.1",
            dependencies: {
              "@actalk/anywrite-core": "workspace:0.5.0",
            },
          },
          null,
          2,
  ...
  it("replaces workspace dependencies before npm pack", { timeout: CLI_PACK_TEST_TIMEOUT_MS }, async () => {
    const packDir = await mkdtemp(join(tmpdir(), "anywrite-cli-pack-"));
    ...
    try {
      ...
      expect(packedPackageJson.dependencies["@actalk/anywrite-core"]).toBe(corePackageJson.version);
      expect(packedPackageJson.dependencies["@actalk/anywrite-studio"]).toBe(studioPackageJson.version);
    } finally {
  ...
  it("packs the studio runtime entry alongside the built frontend", { timeout: STUDIO_PACK_TEST_TIMEOUT_MS }, async () => {
    const packDir = await mkdtemp(join(tmpdir(), "anywrite-studio-pack-"));
