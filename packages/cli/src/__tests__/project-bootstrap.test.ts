...
describe("project bootstrap", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "anywrite-bootstrap-"));
  ...
  it("creates a minimal Studio-first project when none exists", async () => {
    ...
    const config = JSON.parse(await readFile(join(tempDir, "anywrite.json"), "utf-8"));
    expect(config.name).toMatch(/^anywrite-bootstrap-/);
  ...
  it("returns false when the directory is already an AnyWrite project", async () => {
    await writeFile(join(tempDir, "anywrite.json"), "{}\n", "utf-8");
    const { ensureProjectDirectoryInitialized } = await import("../project-bootstrap.js");
