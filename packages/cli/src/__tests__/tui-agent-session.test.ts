...
vi.mock("@actalk/anywrite-core", async () => {
  const actual = await vi.importActual<typeof import("@actalk/anywrite-core")>("@actalk/anywrite-core");
...
describe("tui agent session bridge", () => {
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "anywrite-tui-agent-"));
    vi.clearAllMocks();
