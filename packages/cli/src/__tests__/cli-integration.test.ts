...
import { StateManager } from "@actalk/anywrite-core";
...
const failingLlmEnv = {
  ...
  ANYWRITE_LLM_BASE_URL: "http://127.0.0.1:9/v1",
  ANYWRITE_LLM_MODEL: "test-model",
  ANYWRITE_LLM_API_KEY: "test-key",
};

describe("CLI integration", () => {
  beforeAll(async () => {
    projectDir = await mkdtemp(join(tmpdir(), "anywrite-cli-test-"));
  });
  ...
  describe("anywrite --help", () => {
    it("prints help with command list", () => {
      const output = run(["--help"]);
      expect(output).toContain("anywrite");
  ...
  describe("anywrite init", () => {
    it("initializes project in current directory", () => {
      const output = run(["init"]);
      expect(output).toContain("Project initialized");
    });

    it("creates anywrite.json with correct structure", async () => {
      const raw = await readFile(join(projectDir, "anywrite.json"), "utf-8");
    ...
    it("creates .env file", async () => {
      ...
      expect(envContent).toContain("ANYWRITE_LLM_API_KEY");
    });
  ...
  describe("anywrite init <name>", () => {
    it("creates project in subdirectory", () => {
      const output = run(["init", "subproject"]);
      expect(output).toContain("Project initialized");
    });

    it("creates anywrite.json in subdirectory", async () => {
      const raw = await readFile(join(projectDir, "subproject", "anywrite.json"), "utf-8");
      const config = JSON.parse(raw);
      expect(config.name).toBe("subproject");
    });

    it("supports absolute project paths instead of nesting them under cwd", async () => {
      const absoluteDir = await mkdtemp(join(tmpdir(), "anywrite-cli-abs-init-"));

      try {
        const output = run(["init", absoluteDir]);
        expect(output).toContain(`Project initialized at ${absoluteDir}`);

        const raw = await readFile(join(absoluteDir, "anywrite.json"), "utf-8");
        const config = JSON.parse(raw);
    ...
    it("prints English next steps when initialized with --lang en", async () => {
      const englishDir = await mkdtemp(join(tmpdir(), "anywrite-cli-en-init-"));

      try {
        const output = run(["init", englishDir, "--lang", "en"]);
        expect(output).toContain("Project initialized");
        expect(output).toContain("anywrite book create --title 'My Novel'");
  ...
  describe("anywrite interact", () => {
    it("returns structured JSON for shared interaction mode switches", async () => {
      const initialized = await stat(join(projectDir, "anywrite.json")).then(() => true).catch(() => false);
    ...
    it("binds the requested book when interact is called with --book", async () => {
      ...
      try {
        ...
        await rm(join(projectDir, ".anywrite-session.json"), { force: true }).catch(() => {});
      }
    });
  });

  describe("anywrite config set-model", () => {
  ...
  describe("anywrite doctor", () => {
    ...
    it("treats localhost OpenAI-compatible endpoints as API-key optional", async () => {
      ...
      const configPath = join(projectDir, "anywrite.json");
      const envPath = join(projectDir, ".env");
      ...
      try {
        ...
        await writeFile(envPath, [
          "ANYWRITE_LLM_PROVIDER=openai",
          "ANYWRITE_LLM_BASE_URL=http://127.0.0.1:11434/v1",
          "ANYWRITE_LLM_MODEL=gpt-oss:20b",
          "",
        ].join("\n"), "utf-8");

        const { stdout } = runStderr(["doctor"], {
          env: { ANYWRITE_LLM_API_KEY: "" },
        });
  ...
  describe("anywrite plan/compose", () => {
    beforeAll(async () => {
      const configPath = join(projectDir, "anywrite.json");
      const initialized = await stat(configPath).then(() => true).catch(() => false);
      if (!initialized) run(["init"]);
