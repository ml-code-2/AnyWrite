...
async function saveRawConfig(root: string, config: Record<string, unknown>): Promise<void> {
  await writeFile(join(root, "anywrite.json"), JSON.stringify(config, null, 2), "utf-8");
}
...
async function readEnvConfigSummary(path: string): Promise<EnvConfigSummary> {
  try {
    ...
    const provider = values.get("ANYWRITE_LLM_PROVIDER") ?? null;
    const baseUrl = values.get("ANYWRITE_LLM_BASE_URL") ?? null;
    const model = values.get("ANYWRITE_LLM_MODEL") ?? null;
    const apiKey = values.get("ANYWRITE_LLM_API_KEY") ?? "";
...
export function createStudioServer(initialConfig: ProjectConfig, root: string) {
  ...
  app.get("/api/v1/genres", async (c) => {
    const { listAvailableGenres, readGenreProfile } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/books/:id/truth/:file{.+}", async (c) => {
    ...
    const { isNewLayoutBook } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/services", async (c) => {
    ...
    // Add custom services from anywrite.json
    try {
      const config = await loadRawConfig(root);
  ...
  app.get("/api/v1/project", async (c) => {
    ...
    // Check if language was explicitly set in anywrite.json (not just the schema default)
    const raw = JSON.parse(await readFile(join(root, "anywrite.json"), "utf-8"));
  ...
  let schedulerInstance: import("@actalk/anywrite-core").Scheduler | null = null;
  ...
  app.post("/api/v1/daemon/start", async (c) => {
    ...
    try {
      const { Scheduler } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/logs", async (c) => {
    const logPath = join(root, "anywrite.log");
    try {
  ...
  app.post("/api/v1/books/:id/audit/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const currentConfig = await loadCurrentProjectConfig();
      const { ContinuityAuditor } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/genres/:id", async (c) => {
    ...
    try {
      const { readGenreProfile } = await import("@actalk/anywrite-core");
  ...
  app.post("/api/v1/genres/:id/copy", async (c) => {
    ...
    try {
      const { getBuiltinGenresDir } = await import("@actalk/anywrite-core");
  ...
  app.post("/api/v1/books/:id/detect/:chapter", async (c) => {
    ...
    try {
      ...
      const content = await readFile(join(chaptersDir, match), "utf-8");
      const { analyzeAITells } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/books/:id/detect/stats", async (c) => {
    const id = c.req.param("id");
    try {
      const { loadDetectionHistory, analyzeDetectionInsights } = await import("@actalk/anywrite-core");
      const bookDir = state.bookDir(id);
  ...
  app.post("/api/v1/style/analyze", async (c) => {
    ...
    try {
      const { analyzeStyle } = await import("@actalk/anywrite-core");
  ...
  app.post("/api/v1/books/:id/import/chapters", async (c) => {
    ...
    try {
      const { splitChapters } = await import("@actalk/anywrite-core");
  ...
  app.get("/api/v1/doctor", async (c) => {
    ...
    const { GLOBAL_ENV_PATH } = await import("@actalk/anywrite-core");

    const checks = {
      anywriteJson: existsSync(join(root, "anywrite.json")),
...
export async function startStudioServer(
  ...
  console.log(`AnyWrite Studio running on http://localhost:${port}`);
