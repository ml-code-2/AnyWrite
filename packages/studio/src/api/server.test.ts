...
vi.mock("@actalk/anywrite-core", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@actalk/anywrite-core")>();
  ...
  return {
    ...
    GLOBAL_ENV_PATH: join(tmpdir(), "anywrite-global.env"),
  };
});
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "anywrite-studio-server-"));
    await writeFile(join(root, "anywrite.json"), JSON.stringify(projectConfig, null, 2), "utf-8");
    ...
    loadProjectConfigMock.mockImplementation(async () => {
      const raw = JSON.parse(await readFile(join(root, "anywrite.json"), "utf-8")) as Record<string, unknown>;
      return {
        ...cloneProjectConfig(),
        ...raw,
        llm: {
  ...
  afterEach(async () => {
    ...
    await rm(join(tmpdir(), "anywrite-global.env"), { force: true });
  });

  it("uses the real core bookId validator in the Studio safety mock", async () => {
    const { isSafeBookId } = await import("@actalk/anywrite-core");
  ...
  it("reports config source and detected env overrides for Studio switching", async () => {
    await writeFile(join(root, ".env"), [
      ...
      "ANYWRITE_LLM_BASE_URL=https://project.example.com/v1",
      "ANYWRITE_LLM_MODEL=gpt-5.4",
      "ANYWRITE_LLM_API_KEY=sk-project",
    ].join("\n"), "utf-8");
    await writeFile(join(tmpdir(), "anywrite-global.env"), [
      "ANYWRITE_LLM_PROVIDER=openai",
      "ANYWRITE_LLM_BASE_URL=https://global.example.com/v1",
      "ANYWRITE_LLM_MODEL=gpt-4o",
      "ANYWRITE_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    await writeFile(join(root, "anywrite.json"), JSON.stringify({
      ...projectConfig,
      llm: {
  ...
  it("auto-detects a working custom combination when /models is unavailable", async () => {
    ...
    await writeFile(join(root, ".env"), [
      "ANYWRITE_LLM_MODEL=MiniMax-M2.7",
      "ANYWRITE_LLM_BASE_URL=https://api.minimax.com/v1",
      "ANYWRITE_LLM_API_KEY=sk-minimax",
    ].join("\n"), "utf-8");
  ...
  it("reports async create failures through the create-status endpoint", async () => {
    processProjectInteractionRequestMock.mockRejectedValueOnce(new Error("ANYWRITE_LLM_API_KEY not set"));
    ...
    await expect(status.json()).resolves.toMatchObject({
      status: "error",
      error: "ANYWRITE_LLM_API_KEY not set",
    });
  });
