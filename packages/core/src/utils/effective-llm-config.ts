  ...
  if (!apiKey && input.requireApiKey !== false && !isApiKeyOptionalForEndpoint({ provider, baseUrl })) {
    throw new Error(
      configMode === "studio-project"
        ...
        : "ANYWRITE_LLM_API_KEY not set. Run 'anywrite config set-global' or add it to project .env file.",
    );
  }
...
async function readProjectConfig(root: string): Promise<Record<string, unknown>> {
  const configPath = join(root, "anywrite.json");
  try {
    await access(configPath);
  } catch {
    throw new Error(
      `anywrite.json not found in ${root}.\nMake sure you are inside an AnyWrite project directory (cd into the project created by 'anywrite init').`,
    );
  }
  ...
  try {
    ...
    throw new Error(`anywrite.json in ${root} is not valid JSON. Check the file for syntax errors.`);
  }
}
...
async function applyCliProjectConfig(
  ...
  const envBaseUrl = stringValue(env.ANYWRITE_LLM_BASE_URL);
  const envService = stringValue(env.ANYWRITE_LLM_SERVICE) ?? (envBaseUrl ? guessServiceFromBaseUrl(envBaseUrl) : undefined);
  const envModel = stringValue(env.ANYWRITE_LLM_MODEL);
  ...
  await applyProjectServiceConfig(config, llm, services, input.projectRoot, diagnostics, {
    ...
    envApiKey: allowEnvEndpointOverlay ? stringValue(env.ANYWRITE_LLM_API_KEY) : undefined,
    envBaseUrl: allowEnvEndpointOverlay ? envBaseUrl : undefined,
    envProvider: allowEnvEndpointOverlay ? stringValue(env.ANYWRITE_LLM_PROVIDER) : undefined,
    cli: input.cli,
    env,
  });
}
...
async function applyLegacyEnvConfig(
  ...
  if (env.ANYWRITE_LLM_SERVICE) {
    llm.service = env.ANYWRITE_LLM_SERVICE;
    diagnostics.serviceSource = "env";
  } else if (typeof llm.service !== "string" || llm.service.length === 0) {
    llm.service = "custom";
  }

  if (env.ANYWRITE_LLM_PROVIDER) llm.provider = env.ANYWRITE_LLM_PROVIDER;
  else if (typeof llm.provider !== "string" || llm.provider.length === 0) llm.provider = "custom";
  if (env.ANYWRITE_LLM_BASE_URL) llm.baseUrl = env.ANYWRITE_LLM_BASE_URL;
  if (env.ANYWRITE_LLM_MODEL) {
    llm.model = env.ANYWRITE_LLM_MODEL;
    diagnostics.modelSource = "env";
  }
  if (env.ANYWRITE_LLM_API_KEY) {
    llm.apiKey = env.ANYWRITE_LLM_API_KEY;
...
function applyCommonEnv(
  ...
  if (env.ANYWRITE_LLM_TEMPERATURE) llm.temperature = Number.parseFloat(env.ANYWRITE_LLM_TEMPERATURE);
  if (env.ANYWRITE_LLM_THINKING_BUDGET) llm.thinkingBudget = Number.parseInt(env.ANYWRITE_LLM_THINKING_BUDGET, 10);
  if (env.ANYWRITE_LLM_API_FORMAT) llm.apiFormat = env.ANYWRITE_LLM_API_FORMAT;
  if (env.ANYWRITE_LLM_STREAM) llm.stream = parseBoolean(env.ANYWRITE_LLM_STREAM);
  if (env.ANYWRITE_DEFAULT_LANGUAGE) config.language = env.ANYWRITE_DEFAULT_LANGUAGE;

  const extraFromEnv: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(env)) {
    if (key.startsWith("ANYWRITE_LLM_EXTRA_") && value) {
      extraFromEnv[key.slice("ANYWRITE_LLM_EXTRA_".length)] = parseEnvValue(value);
    }
  }
...
function warnIfStudioIgnoresEnv(layers: LLMEnvLayers, diagnostics: MutableDiagnostics): void {
  ...
  if (Object.keys(ignored).some((key) => key.startsWith("ANYWRITE_LLM_"))) {
    diagnostics.warnings.push("Studio 运行时不会使用 env 中的 ANYWRITE_LLM_* 配置；请在服务配置页保存 Studio 配置。");
  }
}
