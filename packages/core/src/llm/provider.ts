...
export function createLLMClient(config: LLMConfig): LLMClient {
  ...
  const anywriteProvider = getEndpoint(serviceName);
  const modelCard = lookupModel(serviceName, config.model);

  const piApi = resolvePiApi(serviceName, config.apiFormat, (anywriteProvider?.api ?? preset?.api) as PiApi) as PiApi;
  const baseUrl = config.baseUrl || anywriteProvider?.baseUrl || preset?.baseUrl || "";
  const extraHeaders = config.headers ?? parseEnvHeaders();
  const compat = piApi === "openai-completions"
    ? resolveProviderCompat(anywriteProvider, baseUrl)
    : undefined;
  ...
  if (anywriteProvider?.id === "google") piProvider = "google";
  else if (anywriteProvider?.id === "zhipu") piProvider = "zai";
  else if (anywriteProvider?.id === "openrouter") piProvider = "openrouter";
  else if (anywriteProvider?.id === "githubCopilot") piProvider = "githubCopilot";
  else if (anywriteProvider?.api === "anthropic-messages") piProvider = "anthropic";
...
function parseEnvHeaders(): Record<string, string> | undefined {
  const raw = process.env.ANYWRITE_LLM_HEADERS;
...
//
// anywrite 让 writer/validator/architect 各自带 per-call 温度（0.1~1.5），
...
function clampTemperatureForModel(
  ...
  if (!warnedFixedTemperatureModels.has(model)) {
    ...
    console.warn(
      `[anywrite] 模型 "${model}" API 要求 temperature=${locked}，已 clamp（原值 ${requested}）`,
    );
  }
  return locked;
}
...
function wrapLLMError(error: unknown, context?: { readonly baseUrl?: string; readonly model?: string }): Error {
  ...
  if (msg.includes("403")) {
    return new Error(
      ...
      `  3. 账户余额不足\n` +
      `  建议：用 anywrite doctor 测试 API 连通性，或换一个不限制内容的 API 提供方${ctxLine}`,
    );
  }
  if (msg.includes("401")) {
    return new Error(
      `API 返回 401 (未授权)。请检查 .env 中的 ANYWRITE_LLM_API_KEY 是否正确。${ctxLine}`,
    );
  }
  if (msg.includes("429")) {
    return new Error(
  ...
  if (msg.includes("Connection error") || msg.includes("ECONNREFUSED") || msg.includes("ENOTFOUND") || msg.includes("fetch failed")) {
    return new Error(
      ...
      `  2. 网络不通或被防火墙拦截\n` +
      `  3. API 服务暂时不可用\n` +
      `  建议：检查 ANYWRITE_LLM_BASE_URL 是否包含完整路径（如 /v1）`,
    );
  }
...
async function chatCompletionViaCustomAnthropicCompatible(
  ...
  const response = await fetch(`${baseUrl.replace(/\/$/, "")}/messages`, {
    method: "POST",
    headers: {
      "User-Agent": ANYWRITE_USER_AGENT,
...
/** Convert anywrite LLMMessage[] to pi-ai Context. */
...
/** Convert anywrite AgentMessage[] to pi-ai Context (with tool calls/results). */
...
/** Convert anywrite ToolDefinition[] to pi-ai Tool[]. */
...
async function chatCompletionViaPiAi(
  ...
  if (!client.stream) {
    ...
    if (!content) {
      ...
      console.warn(`[anywrite] LLM 非流式响应无文本内容 (${diag})`);
  ...
  if (!content) {
    ...
    console.warn(`[anywrite] LLM 流式响应无文本内容 (${diag})`);
