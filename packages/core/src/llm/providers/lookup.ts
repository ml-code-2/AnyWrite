import type { AnyWriteModel } from "./types.js";
...
export function lookupModel(
  ...
  const matches: Array<{ model: AnyWriteModel; providerId: string }> = [];
...
export function listEnabledModels(serviceId: string): AnyWriteModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
  return provider.models.filter((m) => m.enabled !== false);
}

export function isActiveTextModel(model: AnyWriteModel): boolean {
...
export function listActiveTextModels(serviceId: string): AnyWriteModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
