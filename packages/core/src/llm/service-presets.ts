...
export function resolveServicePreset(service: string): ServicePreset | undefined {
  ...
  return {
    ...
    // piProvider 字段已从 AnyWriteEndpoint 移除（走 provider-to-pi-ai adapter），这里只保留 legacy fallback
...
function toModelInfo(anywriteModel: { id: string; maxOutput: number; contextWindowTokens: number }): ModelInfo {
  return {
    id: anywriteModel.id,
    name: anywriteModel.id,
    contextWindow: anywriteModel.contextWindowTokens,
    maxOutput: anywriteModel.maxOutput,
  };
}
 ...
 * - probe 失败或无 apiKey：fallback 到 provider.models（anywrite bank）
 * - 不再做 ANYWRITE_LLM_MODEL env 补丁（会污染跨 service 菜单；bank 已足够全）
 *
