...
export interface AgentSessionConfig {
  ...
  apiKey?: string;
  /** Allow the read tool to read absolute paths outside projectRoot/books. Defaults to false; set ANYWRITE_AGENT_ALLOW_SYSTEM_READ=1 to enable. */
...
async function runAgentSessionUnlocked(
  ...
  const allowSystemFileRead = config.allowSystemFileRead ?? envFlagEnabled(process.env.ANYWRITE_AGENT_ALLOW_SYSTEM_READ, false);
