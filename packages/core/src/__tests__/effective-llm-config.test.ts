...
describe("resolveEffectiveLLMConfig", () => {
  ...
  async function writeProject(llm: Record<string, unknown>) {
    root = await mkdtemp(join(tmpdir(), "anywrite-effective-llm-"));
  ...
  async function writeSecrets(services: Record<string, { apiKey: string }>) {
    ...
    await writeFile(join(root, ".anywrite", "secrets.json"), JSON.stringify({ services }, null, 2), "utf-8");
  }
  ...
  it("CLI consumer 允许 ANYWRITE_LLM_SERVICE 切换服务，并从 provider bank 推导 baseUrl", async () => {
    await writeProject({
      configSource: "studio",
      service: "google",
  ...
  it("CLI consumer 兼容旧 env：没有 ANYWRITE_LLM_SERVICE 时从 baseUrl 反推 service", async () => {
    await writeProject({
      configSource: "studio",
      service: "google",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ANYWRITE_LLM_SERVICE: "moonshot",
          ANYWRITE_LLM_MODEL: "kimi-k2.5",
          ANYWRITE_LLM_API_KEY: "sk-global-moon",
        },
        project: {
          ANYWRITE_LLM_SERVICE: "deepseek",
          ANYWRITE_LLM_MODEL: "deepseek-chat",
          ANYWRITE_LLM_API_KEY: "sk-project-deepseek",
        },
        process: {
          ANYWRITE_LLM_SERVICE: "zhipu",
          ANYWRITE_LLM_MODEL: "glm-4-flash",
          ANYWRITE_LLM_API_KEY: "sk-process-zhipu",
        },
      },
      requireApiKey: true,
    });
  ...
  it("旧 configSource=env 保持 legacy-env 行为", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ANYWRITE_LLM_PROVIDER: "custom",
          ANYWRITE_LLM_BASE_URL: "https://api.example.com/v1",
          ANYWRITE_LLM_MODEL: "legacy-model",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ...
          ANYWRITE_LLM_MODEL: "legacy-model",
          ANYWRITE_LLM_API_KEY: "sk-env",
          ANYWRITE_LLM_API_FORMAT: "chat",
          ANYWRITE_LLM_STREAM: "true",
        },
        project: {},
        process: {},
      },
  ...
  it("保留旧 ANYWRITE_LLM_EXTRA_* 和 ANYWRITE_DEFAULT_LANGUAGE 行为", async () => {
    await writeProject({
      configSource: "env",
      provider: "openai",
      baseUrl: "https://api.example.com/v1",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        ...
        project: {
          ANYWRITE_LLM_API_KEY: "sk-env",
          ANYWRITE_LLM_EXTRA_top_p: "0.9",
          ANYWRITE_DEFAULT_LANGUAGE: "en",
        },
        process: {},
      },
    });
  ...
  it("CLI override 优先级高于 env", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { ANYWRITE_LLM_SERVICE: "google", ANYWRITE_LLM_MODEL: "gemini-2.5-pro" },
        process: {},
      },
  ...
  it("拒绝不属于最终 service 的模型", async () => {
    ...
    await expect(resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { ANYWRITE_LLM_MODEL: "kimi-k2.5" },
        process: {},
      },
