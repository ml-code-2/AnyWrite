...
describe("project interaction control", () => {
  beforeAll(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "anywrite-project-control-"));
  ...
  it("persists a creation draft across freeform ideation turns", async () => {
    const ideationRoot = await mkdtemp(join(tmpdir(), "anywrite-project-ideation-"));
    await writeFile(join(ideationRoot, "anywrite.json"), JSON.stringify({ language: "zh" }), "utf-8");
