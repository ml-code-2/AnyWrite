---
name: anywrite
description: Autonomous novel writing CLI agent with web workbench (AnyWrite Studio) - use for creative fiction writing, novel generation, style imitation, chapter continuation/import, EPUB export, AIGC detection, and fan fiction. Native English support with 10 built-in English genre profiles (LitRPG, Progression Fantasy, Isekai, Cultivation, System Apocalypse, Dungeon Core, Romantasy, Sci-Fi, Tower Climber, Cozy Fantasy). Also supports Chinese web novel genres (xuanhuan, xianxia, urban, horror, other). Multi-agent pipeline, two-phase writer (creative + settlement), 33-dimension auditing, token usage analytics, creative brief input, structured logging (JSON Lines), multi-model routing, custom OpenAI-compatible provider support, and AnyWrite Studio web UI for visual book management, chapter review, real-time writing progress, market radar, and analytics.
version: 2.3.2
metadata: { "openclaw": { "emoji": "📖", "requires": { "bins": ["anywrite", "node"], "env": ["OPENAI_API_KEY"] }, "primaryEnv": "OPENAI_API_KEY", "homepage": "https://github.com/Narcooo/anywrite", "install": [{ "id": "npm", "kind": "node", "package": "@actalk/anywrite", "label": "Install AnyWrite (npm)" }] } }
---

# AnyWrite - Autonomous Novel Writing Agent

AnyWrite is a CLI tool for autonomous fiction writing powered by LLM agents. It orchestrates a multi-agent pipeline (Radar → Planner → Composer → Architect → Writer → Observer → Reflector → Normalizer → Auditor → Reviser) to generate, audit, and revise novel content with zero human intervention per chapter.
...
anywrite init my-writing-project
...
anywrite config set-global --provider openai --base-url https://api.openai.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
# For compatible/proxy endpoints, use --provider custom and point ONLY to trusted endpoints:
# anywrite config set-global --provider custom --base-url https://your-trusted-proxy.com/v1 --api-key-env OPENAI_API_KEY --model gpt-4o
```

### Multi-Model Routing (Optional)
```bash
# Assign different models to different agents — balance quality and cost
anywrite config set-model writer claude-sonnet-4-20250514 --provider anthropic --base-url https://api.anthropic.com --api-key-env ANTHROPIC_API_KEY
anywrite config set-model auditor gpt-4o --provider openai
anywrite config show-models
```
...
1. **Initialize and create book**:
   ```bash
   anywrite book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000
   # Or with a creative brief (your worldbuilding doc / ideas):
   anywrite book create --title "My Novel Title" --genre xuanhuan --chapter-words 3000 --brief my-ideas.md
   ```
...
2. **Generate initial chapters** (e.g., 5 chapters):
   ```bash
   anywrite write next book-id --count 5 --words 3000 --context "young protagonist discovering powers"
   ```
...
3. **Review and approve chapters**:
   ```bash
   anywrite review list book-id
   anywrite review approve-all book-id
   ```

4. **Export the book** (supports txt, md, epub):
   ```bash
   anywrite export book-id
   anywrite export book-id --format epub
   ```
...
2. **Continue from last chapter**:
   ```bash
   anywrite write next book-id --count 3 --words 2500 --context "protagonist faces critical choice"
   ```
   - AnyWrite maintains 7 truth files (world state, character matrix, emotional arcs, etc.) for consistency
   - If only one book exists, omit `book-id` for auto-detection

3. **Review and approve**:
   ```bash
   anywrite review approve-all
   ```

### Workflow 2.5: Shared Natural-Language Control (Recommended For OpenClaw)

When AnyWrite is being driven by OpenClaw or another external agent, prefer the shared interaction executor instead of stitching together many ad-hoc CLI calls:

```bash
anywrite interact --json --message "continue the current book, but keep the pacing tighter"
anywrite interact --json --message "rewrite chapter 3"
anywrite interact --json --book my-book --message "switch to auto mode"
```
...
Use this when you have an existing novel (or partial novel) and want AnyWrite to pick up where it left off.

1. **Import from a single text file** (auto-splits by chapter headings):
   ```bash
   anywrite import chapters book-id --from novel.txt
   ```
...
2. **Import from a directory** of separate chapter files:
   ```bash
   anywrite import chapters book-id --from ./chapters/
   ```
   - Reads `.md` and `.txt` files in sorted order

3. **Resume interrupted import**:
   ```bash
   anywrite import chapters book-id --from novel.txt --resume-from 15
   ```

4. **Continue writing** from the imported chapters:
   ```bash
   anywrite write next book-id --count 3
   ```
   - AnyWrite reverse-engineers all 7 truth files from the imported chapters
...
1. **Analyze reference text**:
   ```bash
   anywrite style analyze reference_text.txt
   ```
   - Examines vocabulary, sentence structure, tone, pacing

2. **Import style to your book**:
   ```bash
   anywrite style import reference_text.txt book-id --name "Author Name"
   ```
...
1. **Import parent canon**:
   ```bash
   anywrite import canon spinoff-book-id --from parent-book-id
   ```
...
2. **Continue spinoff**:
   ```bash
   anywrite write next spinoff-book-id --count 3 --context "alternate timeline after Chapter 20"
   ```
...
1. **Generate draft only**:
   ```bash
   anywrite draft book-id --words 3000 --context "protagonist escapes" --json
   ```

2. **Audit the chapter** (33-dimension quality check):
   ```bash
   anywrite audit book-id chapter-1 --json
   ```
...
3. **Revise with specific mode**:
   ```bash
   anywrite revise book-id chapter-1 --mode polish --json
   ```
...
anywrite detect book-id

# Deep scan all chapters
anywrite detect book-id --all
```
...
```bash
anywrite analytics book-id --json
# Shorthand alias
anywrite stats book-id --json
```
...
anywrite book create --title "The Last Delver" --genre litrpg --chapter-words 3000

# Or set language explicitly
anywrite book create --title "My Novel" --genre other --lang en

# Set English as default for all projects
anywrite config set-global --lang en
```
...
- Use `anywrite genre list` to see all available genres

### Workflow 11: Fan Fiction

```bash
# Create a fanfic from source material
anywrite fanfic init --title "My Fanfic" --from source-novel.txt --mode canon

# Modes: canon (faithful), au (alternate universe), ooc (out of character), cp (ship-focused)
anywrite fanfic init --title "What If" --from source.txt --mode au --genre other
```
...
```bash
# Via interact
anywrite interact --json --message "把林烬改成张三"
anywrite interact --json --message "rename Lin Jin to Zhang San"

# Via slash command
anywrite interact --json --message "/rename 林烬 => 张三"
```
...
```bash
anywrite interact --json --message "/replace 5 旧文本 => 新文本"
```
...
- Shares the same interaction kernel as `anywrite interact` and Studio

## AnyWrite Studio (Web Workbench)

`anywrite studio` launches a local web UI (default port 4567) that provides a visual interface for all AnyWrite operations:
...
```bash
anywrite studio              # Start on default port 4567
anywrite studio -p 8080      # Start on custom port
```

The right-side **AI Assistant panel** in Studio shares the same interaction kernel as TUI and `anywrite interact`. You can type natural language commands (rename entities, write chapters, audit, export) directly in the assistant panel.
...
```bash
anywrite agent "写一部都市题材的小说，主角是一个年轻律师，第一章三千字"
```
...
```bash
# Explicit
anywrite write next book-123 --count 1

# Auto-detected (if only one book exists)
anywrite write next --count 1
```

### --json Flag
All content-generating commands support `--json` for structured output. Essential for programmatic use:
```bash
anywrite draft book-id --words 3000 --context "guidance" --json
```

### Truth Files (Long-Term Memory)
AnyWrite maintains 7 files per book for coherence:
...
```bash
anywrite write next book-id --count 2 --context "protagonist discovers betrayal, must decide whether to trust mentor"
```
...
```bash
anywrite genre list
anywrite genre show xuanhuan
```

### Create Custom Genre
```bash
anywrite genre create my-genre --name "My Genre"
# Options: --numerical, --power, --era
anywrite genre create dark-xuanhuan --name "Dark Xuanhuan" --numerical --power
```

### Copy Built-in Genre for Customization
```bash
anywrite genre copy xuanhuan
...
|---------|---------|-------|
| `anywrite init [name]` | Initialize project | One-time setup |
| `anywrite book create` | Create new book | Returns book-id. `--brief <file>`, `--lang en/zh`, `--genre litrpg/progression/...` |
| `anywrite book list` | List all books | Shows IDs, statuses |
| `anywrite write next` | Full pipeline (draft→audit→revise) | Primary workflow command |
| `anywrite draft` | Generate draft only | No auditing/revision |
| `anywrite audit` | 33-dimension quality check | Standalone evaluation |
| `anywrite revise` | Revise chapter | Modes: polish/spot-fix/rewrite/rework/anti-detect |
| `anywrite agent` | Natural language interface | Flexible requests |
| `anywrite style analyze` | Analyze reference text | Extracts style profile |
| `anywrite style import` | Apply style to book | Makes style permanent |
| `anywrite import canon` | Link spinoff to parent | For prequels/sequels |
| `anywrite import chapters` | Import existing chapters | Reverse-engineers truth files for continuation |
| `anywrite detect` | AIGC detection | Flags AI-generated passages |
| `anywrite export` | Export finished book | Formats: txt, md, epub |
| `anywrite analytics` / `anywrite stats` | View book statistics | Word count, audit rates, token usage |
| `anywrite radar scan` | Platform trend analysis | Informs new book ideas |
| `anywrite config set-global` | Configure LLM provider | OpenAI/Anthropic/custom (any OpenAI-compatible) |
| `anywrite config set-model <agent> <model>` | Set model override for a specific agent | `--provider`, `--base-url`, `--api-key-env` for multi-provider routing |
| `anywrite config show-models` | Show current model routing | View per-agent model assignments |
| `anywrite doctor` | Diagnose issues | Check installation |
| `anywrite update` | Update to latest version | Self-update |
| `anywrite up/down` | Daemon mode | Background processing. Logs to `anywrite.log` (JSON Lines). `-q` for quiet mode |
| `anywrite review list/approve-all` | Manage chapter approvals | Quality gate |
| `anywrite fanfic init` | Create fanfic from source material | `--from <file>`, `--mode canon/au/ooc/cp` |
| `anywrite genre list` | List all available genres | Shows English and Chinese genres with default language |
| `anywrite genre create <id>` | Create custom genre profile | `--name`, `--numerical`, `--power`, `--era` |
| `anywrite genre copy <id>` | Copy built-in genre to project | For customization |
| `anywrite write rewrite <book> <ch>` | Rewrite a specific chapter | Deletes chapter and later, rewrites from that point |
| `anywrite book update [book-id]` | Update book settings | `--chapter-words`, `--target-chapters`, `--status`, `--lang` |
| `anywrite book delete <book-id>` | Delete book and all chapters | `--force` to skip confirmation |
| `anywrite plan chapter [book-id]` | Generate chapter intent | Preview what next chapter will do before writing |
| `anywrite compose chapter [book-id]` | Generate runtime artifacts | Context, rule-stack, trace for next chapter |
| `anywrite consolidate [book-id]` | Consolidate chapter summaries | Reduces context for long books (volume-level summaries) |
| `anywrite eval [book-id]` | Quality evaluation report | `--json`, `--chapters <range>`. Composite quality score |
| `anywrite studio` | Start web workbench | `-p` for port. Local web UI for book management |
| `anywrite fanfic show [book-id]` | Display parsed fanfic canon | Shows imported source material analysis |
| `anywrite fanfic refresh [book-id]` | Re-import and regenerate fanfic canon | `--from <file>` for updated source material |
| `anywrite interact` | Shared interaction endpoint | `--json`, `--message`, `--book`. Primary entry for OpenClaw |
| `anywrite` (no args) | Launch TUI dashboard | Full-screen Ink + React interactive dashboard |
...
**"book-id not found"**
- Verify the ID with `anywrite book list`
- Ensure you're in the correct project directory

**"Provider not configured"**
- Run `anywrite config set-global` with valid credentials
- Check API key and base URL with `anywrite doctor`
...
- Try `anywrite revise` with `--mode rewrite`
...
2. **Start with style**: If imitating an author, run `anywrite style import` before generation
3. **Import first**: For existing novels, use `anywrite import chapters` to bootstrap truth files before continuing
4. **Review regularly**: Use `anywrite review` to catch issues early
5. **Monitor audits**: Check `anywrite audit` metrics to understand quality bottlenecks
...
8. **Check analytics**: Use `anywrite analytics` to track quality trends over time
9. **Export frequently**: Keep backups with `anywrite export`

## Security & Trust

- **License**: the ClawHub skill descriptor is MIT-0 per platform policy, but the underlying `@actalk/anywrite`, `@actalk/anywrite-core`, and `@actalk/anywrite-studio` npm packages are **AGPL-3.0-only**. Running AnyWrite and distributing modified versions are governed by AGPL. Full source on GitHub for auditability.
- **No install hooks**: npm package has no `preinstall`/`postinstall`/`install` scripts. Install is inert.
- **Local-only file I/O**: all read/write stays inside the project directory (`books/*`, `anywrite.json`, `anywrite.log`). No writes outside the working directory.
- **No telemetry**: AnyWrite does not phone home, collect usage stats, or ship any data to AnyWrite-controlled servers. The only outbound traffic is to the LLM provider endpoint you explicitly configure.
- **Credential handling**: always prefer `--api-key-env <VAR_NAME>` over `--api-key <literal>` so keys never hit shell history. Keys are stored in `anywrite.json` under your project directory — treat it like a secret and add it to `.gitignore` if you commit the project.
...
- **No elevated privileges**: AnyWrite requires no sudo, no global state mutation, no network listening port (Studio binds `localhost:4567` only).

## Support & Resources

- **Homepage**: https://github.com/Narcooo/anywrite
- **Configuration**: Stored in project root after `anywrite init`
- **Truth files**: Located in `books/<id>/story/` per book, with structured JSON in `story/state/`
- **Logs**: Check output of `anywrite doctor` for troubleshooting
